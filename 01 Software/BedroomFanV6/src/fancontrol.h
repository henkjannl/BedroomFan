...
const bool fanOnState = LOW; // Relay is active LOW
const bool fanOffState = HIGH;
...
void switchOnFan() {
  fanStatus=fsOn;
  digitalWrite(RELAY_PIN, fanOnState);
}

void switchOffFan() {
  fanStatus=fsOff;
  digitalWrite(RELAY_PIN, fanOffState);
}
